...
def main():
    ...
    if len(domains['rows']) >= 1:
        for i in domains['rows']:
            ...
            print(f"I: {id_domain} - {domain_url} - {check_domain_in_db['rows']}")
            if not check_domain_in_db['rows']:
                ...
                if expired_domain != "Error":
                    ...
                    print(f"DB COMMIT: {expired_domain['expired_date']} - {id_domain}")
            else:
                print(f"ID_DOMAIN: {id_domain}")
                date_in_bd = db.fetch("SELECT date_time FROM expired_date_domains WHERE id_domain =%s", id_domain)
                print(f"date_in_bd: {date_in_bd}")
                ...
                if days_difference <= days_expired_check:
                    ...
                    if expired_domain != "Error":
                        ...
                        if date_in_bd != expired_domain['expired_date']:
                            db.commit("UPDATE expired_date_domains SET date_time =%s WHERE id_domain=%s", expired_domain['expired_date'], id_domain)
                            print(f"IF DATE IN BD: {expired_domain['expired_date']} - {id_domain}")
                        """Уведомления"""
                        ...
                        for user_telegram in get_list_from_mysql(users_id_for_notification):
                            ...
                            previous_date = db.fetch(
                                "SELECT datetime_notification FROM notifications_expired where id_domain=%s AND "
                                "id_user=%s "
